...
| §17 | GA4 계측 오염·급증 | 🤖 **CI 이관** `watchman-ci.yml` |
...
### §17. GA4 계측 오염·급증 감시 (2026-09-19 추가)

**배경**: 9/17 하루 활성이 16 → 86. 70명은 세션이 로컬 `next start` 에서 돌린 Playwright
실측(localhost)이었고 회장이 GA 화면에서 먼저 봤다. 5/10·8/31·9/14 에 이은 네 번째 자기 오염 —
게이트를 아무리 고쳐도 다음 구멍은 다른 모양이므로 **결과(집계) 쪽에서 매일 본다.**

**판정** (`scripts/watchman/check-ga4-anomaly.mjs`, 판정일 = 2일 전 — 전날은 GA4 미확정)

| 신호 | 등급 |
|------|------|
| 사이트 외 호스트(localhost·*.vercel.app…) 활성 ≥ 1명 | 🟡 게이트 우회 — 어디서 gtag 가 로드됐는지 추적 |
| 사이트 활성 > 직전 7일 중앙값 × 3 이고 +20명 이상 | 🟡 급증 — 근거에 해상도·도시·브라우저 상위 3 서명 첨부 |
| 시크릿 없음 | ⚪ 건너뜀 (로컬) |

**서명 읽는 법**: 해상도가 1280x700·1280x900·393x851·375x900 같은 실측 뷰포트고 도시가 한 곳이면
우리 테스트. 다양한 해상도·도시면 실제 유입(원인 확인 — 기사·커뮤니티 언급 등).

**세션에서**: 🟡 가 뜨면 `gh workflow run ga4-snapshot.yml -f diag=true -f days=5` 로 일자×호스트×
해상도 전체 분해를 뽑고, 우리 것이면 어느 스크립트가 표식 없이 돌았는지 찾아 `diag-browser.mjs`
경유로 고친다. GA 과거 데이터는 못 지우므로 M7 판정은 호스트 필터가 걸린 `ga4-snapshot.mjs` 값만 쓴다.
