...
| 2026-09-17 | **사이드바 스마트 sticky(`StickySidebar`) + 지역 상세 2열 시작점을 탭 내비 바로 아래로 + 탭 포커스 링** — 회장 스크린샷 3장: ① 노트북 뷰포트에서 sticky 사이드바 **하단이 잘린 채** 고정("근거 보기" 버튼 화면 밖) ② 첫 탭 포커스 링 좌상단 모서리 잘림 ③ 시·도 상세는 통계·기후·지원사업이 그리드 **위**에 있어 탭 내비에 들어와도 오른쪽이 비어 있음. 조치: `components/ui/sticky-sidebar.tsx` — 사이드바 높이 ≤ 뷰포트면 CSS sticky 그대로, 넘치면 스크롤 방향에 따라 **아래로는 하단 고정 / 위로는 상단 고정 / 방향 전환 시 그 자리에서 풀어 콘텐츠와 함께 흐름**(relative+margin-top, 전환 틱의 dy 보정으로 휠 한 번에 크게 스크롤해도 어긋나지 않음). 인라인 스타일만 쓰고 여백은 페이지 CSS 의 `top` 에서 `--sticky-top` 을 빼 읽는다. 시·도·시·군·구 모두 `AnchorTabNav` 를 Suspense·그리드 밖으로, 통계·기후(스트리밍)를 좌측 컬럼 첫 항목으로 → 탭 진입 순간부터 사이드바 동반(그리드 시작−탭바 하단 16~32px). 시·도 그리드 2fr/1fr → 5fr/2fr 로 작물·시군구와 통일. 포커스 링은 offset −4 + radius 12 로 컨테이너 clip(radius 16) 안쪽에 | src/components/ui/sticky-sidebar.tsx, sidebar-tabs.module.css, src/app/regions/[id]/page.tsx·page.module.css, src/app/regions/[id]/[sigungu]/page.tsx | **sticky 는 "높이 ≤ 뷰포트" 가 전제고 그 전제는 회장 노트북(≈700px)에서 깨진다** — 900px 실측만 보고 통과시켰던 것. 실측 게이트를 700·900 두 뷰포트 × 250px 스텝 전수 스크롤로: 아래로 갈 땐 `bottom ≥ vh−gap`, 위로 갈 땐 `top == min(헤더+gap, 그리드 하단−h)` 을 매 스텝 검사(위반 0). 방향 전환 첫 틱의 dy 를 버리면 Playwright `wheel(-500)` 한 번에 그대로 남는 게 스크린샷으로 드러났다 — 붙잡혀 있던 동안 콘텐츠가 움직인 만큼 보정 |
