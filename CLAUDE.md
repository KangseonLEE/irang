...
| 2026-09-23 | **search_logs 마크업·스킴 검색어 서버 가드 + GA4 스냅샷 지면별 진입 분포** — ① `javascript:alert(1)`(9/17 13:28 KST) 1건이 search_logs 에 남아 있었음. 세션 기록 대조로 **9/17 보안 점검 세션의 라이브 XSS 실측 5종 중 1건**(19자라 자연어 필터 통과, 나머지 4종은 20자 초과로 skip)으로 확정 — 외부 공격 아님, 다섯 번째 자기 오염 흔적. JSON 백업 후 id 삭제(잔존 0). 현재 코드 반영 경로 전수(React 텍스트 노드·encodeURIComponent·정적 CSS·이스케이프 JSON-LD) 무결, npm audit 0, 9/16 보안 마이그레이션 3종 적용 확인 ② `src/lib/search-log-guard.ts` 신설 — `isMarkupOrSchemeQuery`(꺾쇠·스킴·인라인 핸들러·템플릿·실행 탐침·제어 문자) 를 `/api/search-log` 첫 검증에 추가, `skipped: "markup"` 성공 응답 + fallback 적재, watchman §12 reason 목록 추가, 테스트 20건 ③ `ga4-snapshot.mjs` 에 `assess_entry_click`·`compare_view` × `event_label` 분포 행 추가(9/30 진입점 되돌림 판정 근거) ④ Vercel CLI 59.25.4 | src/lib/search-log-guard.ts, src/app/api/search-log/route.ts, src/__tests__/search-log-guard.test.ts, scripts/watchman/{check-write-activity.ts,ga4-snapshot.mjs} | 회장 "해야 하는 작업 바로 착수". **보안 실측(페이로드 주입)도 실측이다** — openContext 없이 돌린 9/17 XSS 탐침이 DB 에 남았고, DB 게이트는 webdriver 신호를 못 본다(GA 게이트만). 검색어 위생 가드는 사람 표식과 무관하게 스캐너 노이즈까지 함께 거른다. 나머지 안건(9/30 판정·10월 M7·12/15 큐레이션·건수 트리거 3종)은 조건 미도달로 대기 |
| 2026-09-23 | **결과 0건 검색어의 포함어 안내 — "가시오이" → 오이** — 검색 로그의 오이 → 가시오이 → 오이 → 가시오이 왕복을 보고 회장 제안. 실측: 가시오이·방울양배추·흑마늘·대봉감·알타리무 전부 0건에 제안 0(자모 보정은 글자 수 차 ≤1, errata 는 오타 전용, 9/14 힌트는 시드 2건). `findContainedCropNames`(2자 이상 포함, 1자 작물 감·무·배·밤·쌀·콩은 3자 이상 검색어의 **끝글자일 때만** — "감자칩"·"무화과잼" 오탐 방지) + `findContainedRegionNames`(시·도·시·군·구 약칭) → `getNoResultSuggestions`("혹시 이걸 찾으셨나요?" 맨 앞) + `getNoResultHints` 폴백(시드 없으면 포함 작물 → 일러스트 카드). 테스트 7건 | src/lib/data/search-index.ts, src/app/search/page.tsx, src/__tests__/search-index.test.ts | 회장 결재(동결 밖·9/14 결과 없음 정비와 같은 결). **0건의 다수는 오타가 아니라 하위 품종·복합어** — 시드를 늘리는 대신 검색어 안의 실재 이름을 그대로 꺼내면 작물 55종 전체에 바로 적용된다. 띄어쓰기 있는 복합어("가평 펜션")는 이미 토큰 매칭으로 결과가 나오므로 이 규칙은 붙여 쓴 경우만 담당 |
| 2026-09-23 | **포함어 안내 UI 교체 — 박스·알약 → 자동 대체 + 상단 한 줄 (검색 엔진 패턴 학습)** — 회장 "영역을 저렇게 하지 말고 다른 UI, 검색 엔진이 어떻게 처리하는지 학습해라". 네이버·다음·구글·쿠팡·아마존·무신사 Playwright 캡처 + Baymard·NN/g: ① 네이버 "제안 **시솔절임오이**로 검색하시겠습니까?" / 다음 "이것을 찾으시나요? **시골 절임오이** 검색 결과 보기" — 결과 위 **한 줄**, 박스 없음 ② 아마존·구글 — 대안이 하나면 **그 결과를 바로 보여주고** "대신 ~ 검색" 한 줄 ③ Baymard: 대안 1개 = 자동 적용 + 안내, 여럿 = 선택지. 구현: `fallback`(0건 + 포함어 중 결과 있는 첫 항목) → `effectiveQuery` 로 답변카드·패널·결과·연관검색어 전부 대체어 기준, 헤더는 원 검색어 유지 + `.fallbackNotice` 한 줄("‘가시오이’는 아직 없어요. 대신 **오이** 결과 25건이에요. · ‘가시오이’ 정보 요청하기"), 다른 뜻은 인라인 링크. 오타 제안(자모·errata)도 회색 박스·알약 폐기 → "이것을 찾으시나요? **사과**" 한 줄. 로그는 원 검색어 0건으로 남겨 admin 신호 유지. 로컬 prod 375/1280 실측 overflowX 0 | src/app/search/page.tsx·page.module.css | **대안이 하나뿐일 때 "혹시 이거?"라고 되묻는 건 클릭 한 번을 낭비시키는 것** — 검색 엔진은 전부 바로 보여주고 한 줄로 알린다. 레퍼런스는 캡처로 확인(구글·쿠팡은 헤드리스 차단, 네이버·다음·아마존 통과) |
| 2026-09-23 | **[버그] "오이" 검색에 오산시·오미자·오프라인·오대쌀·오금옥이 섞이던 조사 제거 사고** — 회장 "대체 검색 오이에 지역·지자체·용어·교육이 왜 나오는지 모르겠다". 실측: 25건 중 22건이 어느 필드에도 "오이"가 없었다. 원인 `removeKoreanSuffix("오이")` → "이"를 주격 조사로 보고 어간 **"오"**(1자 허용) → 동의어 확장 경로로 "오"가 든 항목 전부 25점 부분 매칭. "경기"→"경"·"완도"→"완"·"보은"→"보"도 동일. 수정: 어간 2자 이상만 인정 + `ENTITY_NAME_SET`(작물·시·도·시·군·구 이름)은 조사 제거 금지. 오이 25→3건(오이·김현 인터뷰·과채류), 완도 →3건, 경기 90건은 전부 "경기" 실포함. 회귀 테스트 4항목("결과 전부가 그 이름을 실제로 포함") + "사과를"→사과 유지 | src/lib/data/search-index.ts, src/__tests__/search-index.test.ts | **"연관이 없어 보인다"는 순위 문제가 아니라 매칭 문제일 수 있다** — 결과마다 "어느 필드로 매칭됐나"를 찍으면 한 번에 드러난다(22/25가 무필드). 한 글자 어간은 한국어 인덱스에서 사실상 와일드카드다. 이 버그는 대체 검색 도입 전부터 있었고 "오이" 직접 검색도 같았다 |
| 2026-09-23 | **검색 매칭 전수 감사(이중 QA) — 와일드카드 결함 3종 추가 수정** — 회장 "정말 중요, 정밀하게 이중 QA". 작물 55·시도 17·시군구 207 이름 + 90일 인기 검색어 40건을 전부 검색해 "그 이름을 실제로 포함하지 않는 결과"를 셌다(`scripts/_diag/_audit-match.ts`). ① 🔴 `injectCropPrefixSpace`가 **1자 작물**(무·감·배·밤·쌀·콩)도 접두 분리 → "무주"→"무 주" **184건**(무·충북농업기술원·안양시…), "무안" 110건, "배송" 217건, "감나무" 91건. 1자 작물 분리 금지 + `ENTITY_NAME_SET` 분리 금지 → 무주 3건 ② 🔴 동의어 사전의 작물→카테고리 확장("포도"→과수, "쌀"·"감자"→식량, "인삼"·"녹차"→특용, "스마트팜"→기술): "포도"에 사과·배·감귤 등 같은 카테고리 20종, "스마트팜"에 농업기술원·시청 44건. 카테고리 토큰 제거(쌀→벼는 유지) ③ 🟡 FAQ `q.includes(kw)` 역포함 매칭에 **1자 키워드**(삼·돈·땅·집·꽃·귤·뜻) → "삼척"·"인삼"에 산양삼 가이드. 1자는 정확 일치일 때만. 회귀 테스트 8건 추가(총 115). 감사 잔여 "무관"은 전부 감사기 오탐(동의어 충북↔충청북도, 조사 제거 정상) | src/lib/data/search-index.ts, src/__tests__/search-index.test.ts | **한 글자는 한국어 인덱스에서 와일드카드다** — 어간(9/23 오전)·접두 분리·FAQ 역포함 세 경로가 전부 같은 결함이었고, 각각 다른 날 다른 목적으로 들어왔다. 매칭 규칙을 추가할 때 "최소 2자" 가드를 기본값으로. 전수 감사는 "결과가 검색어를 포함하는가"라는 단순 기준 하나로 3건을 한 번에 드러냈다 — 회귀 테스트로 고정 |
| 2026-09-23 | **감사 2차 — 일반어 단독 매칭 차단 (FAQ 역포함·복합 OR)** — ① FAQ `q.includes(kw)`: "재배"·"수익" 같은 일반어 키워드 하나로 딸기·사과·블루베리 "재배 정보" FAQ 가 "오이 재배"·"수박 재배지"·"재배x"에도 붙었다. `FAQ_GENERIC_KEYWORDS`(GENERIC_TERMS + 수익·비교·절차·추천·준비·적합·작물·자금·사례·과정·과수·정보) 신설 — FAQ 에 특정어가 있으면 특정어가 맞아야, 전부 일반어인 FAQ(귀농 절차)는 종전대로 ② 복합 쿼리 `scoreItemMulti`: 특정어(전남·오이)가 있는 검색에서 일반어(귀농·재배)만 맞은 항목 제외 — "전남 귀농" 110→56건(전부 전남 실포함), "오이재배" 1위 오이. 전부 일반어("귀농 교육")는 불변. 테스트 5건(총 120, 전체 696) | src/lib/data/search-index.ts, src/__tests__/search-index.test.ts | **OR 검색에서 일반어는 필터가 아니라 잡음 발생기다** — 사용자는 "전남 귀농"을 AND 로 읽는데 인덱스는 OR 로 답했다. 특정어가 하나라도 있으면 그 특정어가 결과의 자격이다. 감사기 잔여 "무관"(충북↔충청북도 동의어·조사)은 오탐 확인 |
| 2026-09-23 | **[🔴 독립 QA 적발] 자동 대체 포함어 매칭 무경계 — 영양제→영양군·강남스타일→강남구·화성동탄신도시→성동구** — 2차 검토자(qa-reviewer, sonnet)가 무작위 지명·브랜드·문장 68건 중 **63건(93%)** 오탐을 실측하고 라이브 재현(영양제 → 대신 영양 결과 6건 + 관련 검색 결과 이중 배너). 내 1차 감사는 실재 이름 검색 → 결과 정합만 봤고 **역방향(임의 검색어 → 대체 후보 정합)** 은 안 봤다. 수정: ① 작물은 검색어 **끝말**일 때만(한국어 복합명사 head 후치: 가시오이·방울양배추·흑마늘·자색고구마), 앞말 1~4자 — 긴 문장 제외 ② 1자 작물(감·무·배·밤·쌀·콩)은 품종 수식어 허용목록(대봉·단·홍시·곶 / 알타리·총각·열 / 나주·신고 / 찹·흑 / 완두·강낭…)일 때만 — 밤나무→무·공감→감·선배→배 차단 ③ **지역명 자동 대체 폐기**(지명은 접두라 영양제·정선희·포항공대와 구분 불가) ④ fallback 중 관련 검색 결과 배너 억제. 부정 테스트 17건 추가(총 123). 스트레스 90건 재실측 오탐 0 | src/lib/data/search-index.ts, src/app/search/page.tsx, src/__tests__/search-index.test.ts | **자동 대체는 결과 없음보다 해로울 수 있다** — 확신 어조로 틀리면 신뢰가 깨진다. 후보 규칙은 양방향으로 감사한다(이름→결과, 임의 입력→후보). 이중 QA 가 실제로 1차가 못 본 축을 잡았다 — 검토자 프롬프트에 무작위·적대 입력 스트레스를 표준 항목으로. 함정: 파이썬 heredoc 안의 따옴표 충돌로 편집 스크립트가 통째로 죽었는데 이전 단계의 미커밋 변경이 그대로 커밋돼(2865e7e) 메시지와 내용이 어긋났다 — 편집은 파일로 쓰고 실행, 커밋 전 `git diff --stat` 확인 |
| 2026-09-23 | **검색 고도화 4종 (회장 "할 수 있는 것 다 진행 + QA 2번")** — ① **관련도 하한선**: 정확 일치(hoist·제목 100점)가 있는 단일어 검색에서 부제 부분 매칭만인 항목(≤18점) 제외 — "사과"에 망고(설명문 "사과의 6배"), "오이"에 과채류 ② **지역 구체성**: `termSpecificity`(시·군·구 3 > 작물·특정어 2 > 시·도 1 > 일반어 0) — 시·군·구가 있는 검색에서 시·도만 맞은 항목 제외("충북 서산" 38→서산 중심, "경기도 가평" 55→가평 중심), 작물 특정어가 맞은 항목은 유지("완도 딸기"의 딸기 카드) ③ **계측** `search_fallback_shown`(label 원검색어>대체어) — 2주 뒤 대체 작물 상세 page_view 대비로 실효 판정 ④ **적대 코퍼스 CI**: `src/__tests__/fixtures/search-adversarial-corpus.ts` NEGATIVE 200여 건(지명+접미·음식·브랜드·인명·문장·감/무/배/밤/쌀/콩 끝 일반어) 전부 후보 0 + POSITIVE 36건 기대 작물 — 새 오탐·품종은 이 파일에 추가 ⑤ 허용목록·코퍼스 갱신을 예약 안건에 등록(12/15 사이클 또는 결과 없는 검색어 누적 시) | src/lib/data/search-index.ts, src/lib/analytics.ts, src/app/search/page.tsx, src/__tests__/fixtures/search-adversarial-corpus.ts, src/__tests__/search-index.test.ts | 하한선은 "정확 일치가 있을 때만" 거는 것이 핵심 — 정확 일치가 없는 검색("도열병")은 부제 매칭이 유일한 통로라 살려야 한다. 코퍼스 첫 실행에서 POSITIVE 3건(녹차·송이)이 작물 목록에 없는 이름이라 탈락 — 기대값도 데이터에 대조해야 한다 |
| 2026-09-23 | **[🔴 2차 독립 QA 적발] 2자 이상 작물 자동 대체 — 여러가지→가지·진심어린사과→사과·차량대파→대파·미국화→국화** — 2차 검토자(sonnet)가 코퍼스 밖 적대 입력을 자체 생성해 라이브 재현. 1자 작물엔 허용목록을 걸었지만 2자 이상은 앞말 1~4자만 보고 통과시켰고, 작물명이 의존명사(가지=종류)·한자어(사과=사죄, 대파=大破, 화=-化)와 겹치는 클래스가 통째로 무방비였다. 수정: `CROP_VARIETY_MODIFIERS`(품종·재배·상태 수식어 70여 개: 가시·방울·흑·자색·미니·노지·유기농·설향·거봉·부사…) + 지역명 접두(`REGION_PREFIX_SET`)일 때만 인정 — 앞말이 사전 항목이거나 2자 이상 항목으로 시작(방울양배추=방울+양)하거나 지역명(청송사과·의성마늘). NEGATIVE 코퍼스에 기능어·한자어 동음 40건 추가. 2차 검토자 부수 확인: 관련도 하한선 494개 단일어 전/후 diff 부작용 0(근채류·엽채류 용어 카드 소실만 ⚪), 일반어 차단 3,612 조합 회귀 0, 계측·UI·SSR 정상. 잔여 🟡: 광주(경기 광주시 vs 광주광역시) 동음은 시·도를 붙여도 안 갈라짐 — 기존 결함, 백로그 | src/lib/data/search-index.ts, src/__tests__/fixtures/search-adversarial-corpus.ts | **허용목록은 예외가 아니라 원칙이어야 한다** — 1자 작물만 위험하다고 본 것이 오판. 확신 어조로 대체하는 기능은 '알려진 형태(품종 수식어·지역명)'만 통과시키는 폐쇄 규칙이 맞다. 2차 검토자가 1차·2차 모두 코퍼스 밖 입력으로 잡았다 — 검토자에게 '코퍼스에 없는 입력 100건 자체 생성'을 명시한 것이 효과 |
| 2026-09-26 | **사용자 요청 "[지원사업 요청] 사과"(9/25) 대응 — 과수 실사업 4건 + 작물 범용 relatedCrops + SP-001·018 정정** — /programs?q=사과 0건·/crops/apple 추천 0건의 원인은 두 겹: ① 사과·과수 사업 자체가 0건 ② 창업자금·청년농·후계농·농지은행·귀농닥터처럼 작물을 가리지 않는 사업도 relatedCrops 가 비어 작물명 검색·작물 상세 추천(55종 중 46종이 0건)에서 전부 빠짐. 조치: `ALL_CROP_NAMES`(범용 6건 = 55종 전부)·`ORCHARD_CROP_NAMES`(과수 카테고리 − 과채 3종) + 신규 SP-050~053(농지은행 과원매매·과원임대차 / 청송 과수생산 지원 / 거창 미래형 사과원 아카데미, 전부 접수 기간 미명시 → 9999 페어) + 작물 상세 추천 정렬(특화 → 접수 중) + SP-001 과원·묘목 용도 문구(시행지침) + SP-018 "수수료 전액 폐지"→농업인 위탁자만 면제(8/29 gov-roadmap 정정이 programs.ts 에 미반영이었음). DB 우선 병합 4건(SP-001·002·011·023)은 `20260926_generic_programs_related_crops.sql` 회장 apply 필요. 라이브 병합 실측 /programs?q=사과 0→6건(apply 후 7) | src/lib/data/{programs,persona-fit,corrections}.ts, src/app/crops/[id]/page.tsx, scripts/check-program-dup.ts, src/__tests__/programs-generic-crops.test.ts, supabase/migrations/20260926_* | 회장 "추천 방향(B+A) 진행, C(0건 화면 브리지)는 대기". **"작물 X 지원사업 0건"은 데이터 부재와 메타데이터 부재를 가른다** — 범용 사업은 있었지만 작물 태그가 없어 검색·추천이 못 찾았고, 그 공백은 사과만이 아니라 46종 전부였다. data-engineer 조사(파일 수정 금지) → CoS 원문 curl 대조 7 URL 후 반영, 탈락 10건 사유 기록. 예약: 충주 2027 과수 시설현대화(접수 ~10/16, 공고 URL 미확보)·거창 10기 공고 시 sourceUrl 교체·거창 청년 귀농인 사과 심화과정 11월 재확인 |
| 2026-09-26 | **관리자 요청 처리 상태 — 즉시 순환 칩 → 목록 선택** — `/admin/requests` 상태 칩이 누를 때마다 대기→완료→반려로 즉시 PATCH 돼 실수 클릭이 곧 변경이었음(회장). 공용 `SelectCombobox`(size sm) 로 교체, 고른 값만 PATCH·실패 시 복원+안내. 테스트 3건, 로컬 dev(`ADMIN_PASSWORD` 임시 주입) 로그인 후 1280·375 캡처 | src/app/admin/requests/status-toggle.tsx·page.module.css, src/__tests__/admin-request-status-toggle.test.tsx | 상태를 바꾸는 컨트롤은 "누름"이 아니라 "고름"에 반응해야 한다(9/7 탭 판정과 같은 결). 함정: `diag-browser.mjs openContext` 가 `x-irang-internal` 헤더를 **모든 origin** 에 붙여 CDN 폰트가 CORS 로 막힌다 — 실측엔 무해하나 콘솔 오류로 오진 가능, 헬퍼 개선 백로그 |
| 2026-09-26 | **작물↔지원사업 연결 3층 구조 — 2층(작물군 국비) 7건 등재, 3층(주산지) 1차 0건** — 회장 "사과 말고 다른 작물도 공통으로". 특화 사업이 붙은 작물 21/55 → 3층으로 정의: ① 범용(`ALL_CROP_NAMES`, 완료) ② 작물군(`GREENHOUSE_VEG`·`FLOWER`·`MUSHROOM`·`ORCHARD_CROP_NAMES`, CROPS 카테고리 도출) ③ 작물 특화(주산지 시군, 수요순). 2층: 정부24 serviceInfo·보조금24 dtlEx 고정 페이지 7건(시설원예 현대화·전략작물직불·고추비가림·임산물 규모화·화훼 습식유통·인삼 시설현대화·에너지절감시설) → **46/55**. 3층 1차(들깨·배추·참깨·수박·가지)는 시군 게시판 curl 검색 불가·모바일 전용 URL·PDF 전용 키워드로 등재 0. `check-cross-reference` D-1/D-2 를 정규식 → 런타임 PROGRAMS 로 교체 | src/lib/data/{programs,persona-fit}.ts, scripts/{check-program-dup,check-cross-reference}.ts | **정부24 고정 페이지는 한글이 숫자 엔티티(`&#50896;`)라 grep 이 0건으로 나온다 — `html.unescape` 후 대조**. `serviceInfo` 는 HTTP 200 + `<title>서비스를 찾을 수 없습니다</title>` 소프트 404 가 있어(전략작물직불) `dtlEx` 계열도 인정. 상대 표기 신청 시기("매년 12월"·"전년도 4~6월")는 9999 페어라 국비 상설 사업 7건이 전부 "모집예정" — 상시·연중 탭 편입은 후속 판단. 잔여 9종(고구마·배추·마늘·양파·무·대파·생강·당근·메밀)은 개인 농가 대상 국비 사업 부재 — 시행지침서 PDF·광역 농업기술원 상설 페이지가 다음 조사 경로 |
| 2026-09-26 | **잔여 9작물 광역 사업 4건 — 특화·작물군 사업 55/55 완성** — 농식품부 「2026 시행지침서」(693p) 220개 사업 전수 대조로 **9종 개인 농가 대상 국비 사업 부재 확증**(배추·무·마늘·양파 명시 3건은 지자체·법인·종자업자 전용). 광역으로 덮음: 전남 연작장해 경감제(노지채소 7종·상시)·강원 고랭지채소(무·배추, 도 상설 페이지)·전북 밭농업직불(밭작물 15종)·제주 암반제거. `FIELD_CROP_NAMES`·`OPEN_FIELD_VEG_CROP_NAMES` 상수. 제외: 부산(대상·내용 불일치)·우수종묘(3억 하한)·B급 5건(title 만 작물) | src/lib/data/{programs,persona-fit}.ts | **"국비에 없다"는 지침서 목차 전수로 확정한다** — 보조금24 검색만으로는 부재 증명이 안 된다. 광역 사업은 보조금24 `dtlEx` 에 소관기관 prefix(전남 646·전북 645·제주 650)로 고정 페이지가 있다. 함정: 전남은 정부24 소관기관이 "전남광주통합특별시"로 뜨지만 PROVINCES SSOT 는 "전라남도" 유지. 3층(주산지 시군) 조사는 별도 진행 중 |
| 2026-09-26 | **[리디자인] 통합검색 결과 유형별 개편 B안 Phase A·B·C (동결 예외, 회장 결재)** — 회장 "다 카드 리스트로만 표시" → UX·프론트 read-only 병렬 검토 → A/B/C 비교표 → B 전체 결재. 사실 정정: 유형별 섹션·상한·더보기는 이미 있었고 **10개 유형의 카드 뼈대가 같은 것**이 문제. A: `searchAllGrouped()` 로 직답 블록(읍면동 안내·정확 일치 hoist·딥링크·FAQ·교차 카드) 분리(`searchAll` 은 래퍼, 테스트 130건 무수정) / **[버그] 지역 카드 id 하이픈 split 로 시·군·구 31건+구 32건이 단순 카드 강등**("중구" 1/6 → 6/6, `region-lookup.ts` href 파싱) / 계측 `search_result_click`. B: 카드 `<a>` → `<article>` + 제목 stretched link(보조 액션 가능), 공용 배지 4종 교체(자체 배지 클래스 삭제). C: 지역 시·도 그룹(12건 696px, 항목당 −48%)·인터뷰 인물 카드(일러 19명)·교육/행사 일정형·용어 `<dl>`·센터 tel/홈페이지·작물 2열은 설명 잘림으로 1열 유지. 8검색어 총건수·BAILOUT 3 전/후 동일, 742 테스트. **D**: 빈 검색 화면 인기 검색어 8회 전수 스캔 → 0, 대체 검색 3 → 2회, 연관 검색어 캐시, 지역 파서·렌더러 Map 캐시(렌더 −34~57%), 죽은 CSS 29줄 — DOM 지문 4,115요소 diff 0 | src/lib/data/search-index.ts, src/app/search/*, src/components/search/{result-card,region-result-group,region-lookup,interview-result-card,glossary-result-list}.*, src/components/analytics/search-result-tracker.tsx, src/__tests__/{search-index,search-result-tracker,search-result-layout}.test.* | **섹션 순서는 "첫 등장 순" 고정** — 유형별 고정 정책으로 바꾸면 hoist 신호가 조용히 소실된다(빌드·테스트 통과, 화면만 퇴화). 함정 2건: 병렬 작업 중 sub-agent 의 `git stash -u` 가 다른 에이전트 변경을 4분간 걷어냈다(손실 0 이었으나 **병렬 세션에선 stash 금지, 워크트리로 비교**) / 모델 응답 지연으로 sub-agent 2건이 600초 무진행 중단 → `SendMessage` 재개로 컨텍스트 유지한 채 마무리 |
| 2026-09-26 | **3층 주산지 시군 특화 사업 8건 (SP-070~077)** — 고구마(당진 무병묘 종순)·배추(진도 부직포)·마늘·양파(고흥 방제약제, 합천 5종 상설)·대파·생강(서산 가격안정, 진도 생강 70%)·당근(제주 가격안정관리제)·메밀(제주 토종농작물). 무는 5개 주산지 전부 현행 유효 사업 부재 → SP-062 로 충분. 태백 고랭지채소(3년 반복)는 SP-062 의 시·군 창구라 미등재. 탈락 30여 건 사유 기록(재배 축소·휴경 보상, 단체 전용, 단발, 첨부 PDF 전용 키워드, 2019 일회성 보도자료) | src/lib/data/{programs,persona-fit}.ts | 시군 게시판은 검색 파라미터가 무시되므로 **목록 GET 순회 + 전년도 공고 대조**가 정석(브라우저 불필요했음). URL 형식 함정: 진도 `B0052.cs?m=23&act=read&articleId=`(다른 형식은 title 미표기), 해남은 4개 파라미터 전부 필요. **"무사마귀병"≠무** — 1자 작물은 병명·행정용어(업무·공무원)에도 걸린다. 사이클: 당진 4월·진도 4월/11월·고흥 11~12월·서산 파종기·제주 9~10월/8월 말 — 12/15 큐레이션 때 접수 일자 반영 후보 |
| 2026-09-27 | **검색 지역 섹션 재설계 + 연례 창구형 접수 시기 표기 + C안 폐기** — ① 회장 "지역 UI 너무 쓰기 불편": 40px 압축 행·"2건 이상만 묶기"라 1건짜리가 순위 자리에 끼어 규칙이 안 읽혔음. CSS 주입 시안 4종(현재 696/행 확대 1,142/카드 1,618/칩 564px) 캡처로 결재 → **행 확대 + 전부 시·도 묶기 + 5건 페이지네이션**(섹션 514px, `search_region_page` 계측) ② "국비 상설을 연중 탭에?" 재검증: 11건 원문 중 연중·수시는 전남 연작장해뿐(정부24 과원규모화도 "접수기관별 상이") → 상시 탭 편입은 오해. 대신 `applicationCycle` 필드(24건)로 "공고 발표 예정" → 배지 "정기 접수" + 기간 자리 "매년 12월 시·군·구 접수". 9999 페어엔 DeadlineBadge "상시" 제거 ③ C안(작물명 0건 화면 브리지)은 55종 전부 사업이 붙어 폐기 | src/components/search/region-result-group.*, src/app/search/page.tsx, src/lib/{format,program-status}.ts, src/lib/data/programs.ts, src/app/programs/**, src/components/{ui/deadline-badge,programs/application-timeline}.tsx | **"쓰기 불편"은 밀도 문제가 아니라 규칙 가독성 문제였다** — 예외(1건짜리 압축)가 묶음 규칙을 숨겼다. 시안은 라이브 DOM 에 CSS 만 입혀 4장 캡처(세로 px 병기)로 한 라운드에 결재. "상시"는 원문에 연중·수시가 있을 때만 — 연례 창구는 시기 문구가 정답. 잔여: 직답 블록 "중구" 6건이 시·도마다 1곳 헤더로 686px(회장 확인 대기). 이어서 회장 "지원사업도 5개 넘으면 페이지네이션" → 페이저를 `section-pager` 로 추출해 지원사업 섹션 5건씩(계측 `search_section_page` 로 일반화) |
