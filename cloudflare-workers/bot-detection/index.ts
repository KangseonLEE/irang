/**
 * Cloudflare Worker — Bot Detection (Sprint D D1 PoC, 2026-05-19)
 *
 ...
 *
 * KV cache:
 *   key:   `${ip}:${ua_hash}`  (ua_hash = ua 첫 32자 base16 hash)
 *   value: "block" | "allow" | "verified"
 *   TTL:   3600s (1h)
 *   1h 안에 같은 ip+ua 재요청 → KV 단일 read (< 5ms) 후 즉시 응답
 *
 * 비용 (Workers Free 100k req/day · KV Free 100k read + 1k write/day):
...
export interface Env {
  // BOT_DETECTION_CACHE: KVNamespace;  // D2에서 binding 추가
}
...
const HEADLESS_BROWSER_PATTERNS: RegExp[] = [
...
const VERIFIED_BOT_PATTERNS: RegExp[] = [
...
type Decision = "block-403" | "block-503" | "allow";
