...
// ---------------------------------------------------------------------------
// Import SIGUNGUS and GUS from the project's canonical data sources
// ---------------------------------------------------------------------------
interface Sigungu {
  id: string;
  name: string;
...
async function loadGus(): Promise<GuDistrict[]> {
  const mod = await import(
    resolve(PROJECT_ROOT, "src/lib/data/gus.ts")
  );
  return mod.GUS as GuDistrict[];
}
...
/**
 * For a sub-district path (e.g. "수원시 장안구"), find the matching GuDistrict.
 * Returns null if the path is not a sub-district or no matching gu is found.
 */
function findGuForPath(
  pathId: string,
  gus: GuDistrict[],
  provinceId: string,
): GuDistrict | null {
  ...
  const guName = parts[parts.length - 1]; // e.g. "장안구"

  // Find matching gu by name and province
  return gus.find(
    (g) => g.name === guName && g.sidoId === provinceId,
  ) ?? null;
}
...
// ---------------------------------------------------------------------------
// District map output (구 단위 — 시 내 구 경계)
// ---------------------------------------------------------------------------

interface DistrictMapEntry {
  guId: string;
  name: string;
  path: string;
}
...
function generateDistrictFileContent(
  ...
  const lines: string[] = [
    ...
    "",
    `export const VIEWBOX = ${JSON.stringify(viewBox)};`,
    `export const SIGUNGU_ID = ${JSON.stringify(sigunguId)};`,
    "",
...
function generateDistrictIndexContent(districtIds: string[]): string {
  const lines = [
    "// Auto-generated by scripts/generate-province-maps.ts",
    "",
    "import type { GuMapLocation } from './" + districtIds[0] + "';",
    "",
    "export type { GuMapLocation };",
    "",
    "/** 구 지도가 있는 시군구 ID 목록 */",
    `export const DISTRICT_MAP_IDS = ${JSON.stringify(districtIds)} as const;`,
    "",
    "// Dynamic import helper for code-splitting",
...
async function main() {
  ...
  console.log("Loading GUS data...");
  const allGus = await loadGus();
  console.log(`  Loaded ${allGus.length} gu entries.`);
  ...
  const districtOutDir = resolve(PROJECT_ROOT, "src/lib/data/district-maps");
  if (!existsSync(districtOutDir)) {
    mkdirSync(districtOutDir, { recursive: true });
  }

  let totalFiles = 0;
  let totalWarnings = 0;

  // Collect district maps across all provinces
  // Key: sigunguId (e.g. "suwon"), Value: { viewBox, entries }
  const districtMaps = new Map<
    string,
    { viewBox: string; entries: DistrictMapEntry[] }
  >();
  ...
  for (const province of PROVINCES) {
    ...
    for (const p of paths) {
      ...
      // Also check for 구-level match (unmerged paths)
      const gu = findGuForPath(p.id, allGus, province.id);
      if (gu) {
        const dm = districtMaps.get(gu.parentSigunguId);
        if (dm) {
          dm.entries.push({ guId: gu.id, name: gu.name, path: p.d });
        } else {
          districtMaps.set(gu.parentSigunguId, {
            viewBox,
            entries: [{ guId: gu.id, name: gu.name, path: p.d }],
          });
        }
      }
    }
  ...
  // Write province-maps index.ts barrel
  ...
  for (const [sigunguId, dm] of districtMaps) {
    ...
    const content = generateDistrictFileContent(dm.viewBox, sigunguId, dm.entries);
    const filePath = resolve(districtOutDir, `${sigunguId}.ts`);
    writeFileSync(filePath, content, "utf-8");
  ...
  if (districtIds.length > 0) {
    const districtIndexPath = resolve(districtOutDir, "index.ts");
    writeFileSync(districtIndexPath, generateDistrictIndexContent(districtIds), "utf-8");
    console.log(`\nWrote ${districtIndexPath} (${districtIds.length} cities with district maps)`);
  }

  console.log(`\nDone. ${totalFiles} province files generated, ${districtMaps.size} district maps, ${totalWarnings} warnings.`);
}
